  ...
  const configs = Object.keys(requiredConfigs)
    .filter((key) => {
      if (!currentConfigs[key]) {
        console.log(chalk.yellow(`Secret missing: [${key}: ${requiredConfigs[key]}]`));
      }
...
const promptRequiredConfigs = ({ ssmPath, requiredConfigs, noninteractive }) => {

  return readRemoteConfigs({ ssmPath })
    .then(currentSecrets => {
      if (noninteractive === true) {
        console.log(chalk.gray('Running on non interactive mode..'));
        return getAllExistingConfigs(requiredConfigs, currentSecrets);
      }
      ...
      const schema = map(requiredConfigs, (value, key) => {
