...
describe('getSettings', () => {
  it('should throw an error when the provider is valid but the configuration is incorrect', () => {
    const getSettings = makeGetSettings({
      settingsFilePath: path.resolve(process.cwd(), './mocks/invalid-dynamodb-provider.yml')
    });
  ...
  it('should throw an error when the provider is invalid', () => {
    const getSettings = makeGetSettings({
      settingsFilePath: path.resolve(process.cwd(), './mocks/invalid-provider.yml')
    });
  ...
  it('should throw an error if the file type is not supported', () => {
    const getSettings = makeGetSettings({
      settingsFilePath: path.resolve(process.cwd(), './mocks/invalid-file-type.json')
    });

    return expect(getSettings()).rejects.toEqual(new Error(`Could not find oprah.yml in the following directory - ${process.cwd()}`));
  });

  it('should interpolate the yaml file with the variables provided', () => {
    const getSettings = makeGetSettings({
      settingsFilePath: path.resolve(process.cwd(), './mocks/dynamodb-provider.yml'),
      variables: {
        stage: 'test'
      }
    });
    ...
    .then(settings => {
      expect(settings).toEqual({
        ...
        secret: {
          keyId: 'arn:aws:kms:us-east-1:1234:key/65def1bd-e786-4334-a17f-4cc0af72fed3',
