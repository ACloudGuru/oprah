...
const makeGetAllParameters = ({ getLatestVersion }) => ({ parameterNames }) => {
  const accumulateParameters = parameters =>
    parameters.reduce((acc, parameterObject) => {
  ...
  return Bluebird.all(
    parameterNames.map(parameterName =>
      getLatestVersion({ parameterName }).then(Item => ({
        Name: parameterName,
