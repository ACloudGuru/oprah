...
describe('generateNextVersion', () => {
  it('should return initial value if currentVersionNumber is not set', () => {
    ...
    expect(generateNextVersionNumber({ currentVersionNumber })).toEqual(
  ...
  it('should throw if currentVersionNumber is nan', () => {
    ...
    expect(() => generateNextVersionNumber({ currentVersionNumber })).toThrow(
      `Cannot generate next version number for 'abc'. Current version number must be number.`
    );
  });
});
