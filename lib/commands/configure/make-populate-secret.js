...
const makePopulateSecret = ({ parameterStore, promptRequiredConfigs }) => ({
  settings,
  interactive,
  missingOnly
}) => {
  ...
  if (!path) {
    return Bluebird.reject(
      new Error('Please specify ssmPath for populateSecret')
    );
  }
  ...
  return promptRequiredConfigs({
    ...
    const parameters = mapKeys(secrets, (_, key) => `${path}/${key}`);
