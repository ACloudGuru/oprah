...
const listConfigurations = (path, message) => {
...
const makeRun = ({ params }) => () => {
  ...
  if (params.list === true) {
    ...
    return configPromise
      .tap(configs => listConfigurations(configs.config && configs.config.path, 'Configs:'))
      .tap(configs => listConfigurations(configs.secret && configs.secret.path, 'Secrets:'))
  }
